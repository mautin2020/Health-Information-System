<?php #  add_pharmacist.php 
// This page is for adding pharmacist.
// This page is accessed through manage_users.php. 

$page_title = 'Add Pharmacist';
...
// Check if the form has been submitted:
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if ($_POST['sure'] == 'Yes') { // Add as Pharmacist
            ...
            // Make the query:
            $q = "UPDATE users SET user_level=4 WHERE user_id=$id LIMIT 1";
            $r = @mysqli_query($dbc, $q);
            if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.

                // Print a message:
                echo '<p>The user has been added as a Pharmacist</p>';
            
            } else { // If the query did not run OK.
                echo '<p class="error">The user could not be added as a nurse due to a system error.</p>';
                // Public message.
        ...
        echo '<p>The user has NOT been added.</p>';
            ...
            if (mysqli_num_rows($r) == 1) {// Valid user ID, show the form.
                ...
                // Get the user's information:
                   ...
                   echo' <br />
                   <div>Are you sure you want to add this user as a Pharmacist?</div>';
                      // Create the form:
                     echo '<form action="add_pharmacist.php" method="post">
